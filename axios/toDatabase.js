...
const toDatabase = async (req, res) => {
    ...
    for(var key in datas.result){
        ...
        var query = `
        INSERT INTO 
            ${schema}.meet_texts(meet_title,start_time, speaker_label, end_time, result) 
        VALUES('${req.body.meet_title}','${temp.start_time}','${temp.speaker_label}','${temp.end_time}','${temp.result}')
        `
        await dbPool(query);
    }
